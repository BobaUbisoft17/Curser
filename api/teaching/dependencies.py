...
class CourseDataIsValid:
    ...
    def __init__(self, session: AsyncSession, course_data: CourseDataForVerification):
        self.session = session
        self.course_data = course_data

    async def validate(self) -> CourseDataForVerification:
        if self.course_data.name is not None and await self.course_name_is_not_available():
